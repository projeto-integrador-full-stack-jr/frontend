...
const Tabs = ({
  ...
  return (
    <div
      className={clsx(
        'mb-6 flex w-full items-center rounded-lg bg-gray-200',
        icon ? 'p-2' : 'px-0'
      )}
    >
      <button
        ...
        className={`w-1/2 cursor-pointer rounded-md py-3.5 text-center text-sm font-semibold transition-colors ${
          currentPage === 'tab1'
            ? tabsVariants[variant][0]
            : tabsVariants[variant][1]
        }`}
      >
        {icon && isIconForFirst && iconPosition === 'left' && (
          <span className="inline-flex">{icon}</span>
        )}

        {label1}
        {icon && isIconForFirst && iconPosition === 'right' && (
          <span className="inline-flex">{icon}</span>
        )}
      </button>
      ...
      >
        {icon && isIconForSecond && iconPosition === 'left' && (
          <span className="inline-flex">{icon}</span>
        )}
        {label2}
        {icon && isIconForSecond && iconPosition === 'right' && (
          <span className="inline-flex">{icon}</span>
        )}
      </button>
    </div>
